...
def extractRecords(recordsJson):
    recordsToUpdate = []
...
def getLeadChangesSince(minutes):
    ...
    recordsToUpdate = []
    recordsToUpdate.append(extractRecords(changesJson))
    ...
    if changesJson['TotalPages'] > 1:
        ...
        while pageIter <= changesJson['TotalPages']:
            ...
            # Extract the records from the API data
            leadsJson = response.json()
            recordsToUpdate.append(extractRecords(leadsJson))
            pageIter += 1
...
def upsertIntoBigQuery(table_id):
    ...
    sql = """
        ...
        WHEN MATCHED THEN
          UPDATE SET value = staging.value
